...
class MotorController(Node):
    def __init__(self):
        ...
        self.left_motor_in1 = 7  # GPIO pin for left motor direction
        self.left_motor_in2 = 29
        self.left_motor_en_pin = 15  # GPIO pin for left motor EN
        
        # Right motor connections
        self.right_motor_in1 = 31  # GPIO pin for right motor direction
        self.right_motor_in2 = 32
    ...
    def listener_callback_left_ticks(self, msg):
        ...
        if elapsed_time > 0.0:
            self.speed_left = float(3.14159265359 * self.wheel_diameter * diff_ticks / (131.25 * 64 * 0.03))
        else:
            self.speed_left = 0.0
            self.setpoint_left = 0.0
        ...
        if elapsed_time > 0.0:
            ...
            self.pwm_left = int(abs(self.pwm_left * 10 + 60))

            if self.pwm_left > 85:
                self.pwm_left = 85
            elif self.pwm_left < 50:
                self.pwm_left = 0
    ...
    def listener_callback_right_ticks(self, msg):
        ...
        if elapsed_time > 0.0:
            self.speed_right = 3.14159265359 * self.wheel_diameter * diff_ticks / (131.25 * 64 * 0.03) # 30ms
        else:
        ...
        if elapsed_time > 0.0:
            ...
            self.pwm_right = int(abs(self.pwm_right * 10 + 60))

            if self.pwm_right > 85:
                self.pwm_right = 85
            elif self.pwm_right < 50:
                self.pwm_right = 0
